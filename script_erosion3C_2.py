...
def erosion3C(T,P,U2,SF2,rhos0,rhos0ind,rhosmax,tDR):
    '''
    Calcul 1D de la masse erodee pour 3 couches au cours du temps
    Parameters
    ----------
    T : numpy array (len,)
        temperature [K]
    ...
    for i in range(nb_date):
        ...
        if test and ustar[i]>ustarT[couche] and rhos[i,couche]<rhosmax: #si CC erodable et existante
            qsalt=(ustar[i]**2-ustarT[couche]**2) / (3.25*g*0.08436*ustar[i]**2.27)
            Ep = qsalt * C_D * ustar[i] * np.log(10/z0) / vk
            ER=Ep * P[i]*100/(287*T[i]) * (1-fracdtF) * (1-fracdtI) * dt
        ...
        if rhos[i,1]==rhosmax: #CI-->CC
            ...
            AC[i,1]=0
            rhos[i,0]=rhosmax
        #si cette opération est effectuée, CI a été densifiée donc CF est vide
        
        else:
            if rhos[i,2]==rhosmax and np.isnan(rhos[i,1]): #CF-->CC ; (CI est vide)
                rhos[i,2]=np.nan
                AC[i,0]+=AC[i,2]
                AC[i,2]=0
                rhos[i,0]=rhosmax
    
            elif rhos[i,2]>rhos02: #CF-->CI
                if np.isnan(rhos[i,1]): #CI est vide
                    rhos[i,1]=rhos[i,2]
                else:
