...
def plotplurian1C(choice,date,string,SF2,rhos,AC,EReff):
    ...
    plt.subplot(n,1,1)
    plt.plot(date,SFcumul,color='b',label='acc. brute')
    plt.plot(date,AC,color='k',label='acc. nette')
    plt.ylim(-20,250)
    plt.ylabel('Accumulation \n [mmwe]')
    plt.legend()
    plt.gca().xaxis.set_ticks(range(2010,2020+1,1))
    plt.gca().yaxis.set_ticks(range(0,251,50))
    plt.gca().xaxis.grid()
    plt.gca().yaxis.grid()
    plt.ylim(-15,251)
    plt.xticks(color='w')
    ...
    plt.savefig(r"C:\Users\Toshiba\Documents\stage L3\Documents\figures\1C_2010_2020"+string+".png",bbox_inches="tight")
    plt.show()
    if choice!=0:
        plt.pause(1)
        plt.close()
...
def plotsensibilite1C(var,strvar,string,Lrhos_mean,LACfinal,Loccurence_ER,LEReff_moy):
    size=13
    ...
    plt.subplot(1,2,1)
    plt.plot(var,Lrhos_mean,marker,label='masse volumique \n (moyenne temporelle)')
    plt.grid()
    plt.ylabel('ρ [kg/m3]')
    plt.xlabel(strvar)
    plt.legend()
    ...
    plt.savefig(r"C:\Users\Toshiba\Documents\stage L3\Documents\figures\1C_2010_2020"+string+".png")
    plt.show()

##preparation plot annuel
def plotannuel1C(choice,mois,string,U2,SF2,rhos,AC,EReff):
    SFcumul=[]
    a=0
    for x in SF2:
        a+=x
    ...
    plt.subplot(n,1,1)
    plt.plot(mois,SFcumul[:365*24],color='b',label='acc. brute')
    plt.plot(mois,AC[:365*24],color='k',label='acc. nette')
    plt.ylabel('Accumulation \n [mmwe]')
    plt.legend()
    ...
    plt.plot(mois,EReff[:365*24],color='r',label='Erosion effective totale')
    plt.ylabel('[mmwe/h]')
    plt.legend()
    plt.gca().xaxis.set_ticks(range(1,14,1))
    plt.gca().xaxis.grid()
    ...
    plt.subplot(n,1,3)
    plt.plot(mois,rhos[:365*24],color='k',label='masse volumique neige')#marker=',',linestyle='None',
    plt.ylabel('ρ [kg/m3]')
    plt.xlim(0.4,13.6)
    plt.legend()
    plt.gca().xaxis.set_ticks(range(1,14,1))
    ...
    plt.savefig(r"C:\Users\Toshiba\Documents\stage L3\Documents\figures\1C_2010"+string+".png",bbox_inches="tight")
    plt.show()
    if choice!=0:
        plt.pause(1)
        plt.close()
