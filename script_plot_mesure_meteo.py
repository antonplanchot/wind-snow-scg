...
plt.subplot(nb,1,1)
plt.plot(Ltime[moitpas:-moitpas],WDIRliss,markersize=2,linestyle='none', marker='*',label='capteur 1')
plt.plot(Ltime[moitpas:-moitpas],WDIR_2liss,markersize=2,linestyle='none',marker='*',label='capteur 2')
...
step=12.5
ran=[x/10+step/2 for x in range(0,3600,125)]
#ran=range(45,285+1,30)
J=[k for k in ran]
J2=[0 for _ in ran]
J3=[k+step/2 for k in ran]
...
for k in range(82277+moitpas,82277+4950-moitpas):
    ...
    while test:
        if J[i]<=WDIRliss[compt]<J[i+1]:
            J2[i]+=int(round(-ERefftot[k],3)*1000)
            test=False
        i+=1
    compt+=1
...
#attention, c'est juste un histogramme du vent ! l'intensité de l'érosion n'est en fait pas prise en compte...
ax = new_axes()
...
plt.text(0.45*np.pi,57,'Erosion [% du total]',fontsize='20')
plt.text(0.77*np.pi,75,'(b)',fontsize=size+15)
...
ax.bar(wd, ws, normed=True, opening=0.8, edgecolor='white')
#plt.title('Erosion cumulée modélisée [mm w.e] selon les directions mesurées du vent \n du 22/5/2019 au 14/12/2019 au Col Sud')
plt.text(0.45*np.pi,57,'Erosion [%]',fontsize='20')
#plt.text(0.77*np.pi,75,'(b)',fontsize=size+15)
plt.savefig("C:/Users/Toshiba/Documents/stage L3/Documents/figures/rose_des_erosions.png",bbox_inches="tight")
plt.show()
...
ax.bar(wd, ws, normed=True, opening=0.8, edgecolor='white')
ax.set_legend()
plt.text(0.45*np.pi,43,'fréquence [%]',fontsize='20')
plt.text(0.77*np.pi,56,'(a)',fontsize=size+15)

plt.show()

#___________________________________________________
#moyenne journalière vent 2010
M=[]
for k in range(365):
    M.append(np.mean(U[k*24:(k+1)*24]))

plt.plot([k for k in range(365)],M)
plt.grid()
plt.show()

#moyenne hebdomadaire vent 2010
M2=[]
for k in range(52):
    M2.append(np.mean(U[k*24*7:(k+1)*24*7]))
    
plt.plot([k for k in range(52)],M2)
plt.grid()
plt.show()   
